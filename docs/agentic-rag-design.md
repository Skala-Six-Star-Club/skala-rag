...
| RAG 및 재검색 전략 | Pre-retrieval Query Rewriting + `role=target` 사전 필터 + Dense Top-5, 재검색 없음(단일 패스) |
...
| RAG 및 재검색 전략 | Pre-retrieval Query Rewriting + `role=target` 사전 필터 + Dense Top-5, 웹 검색 병행, 재검색 최대 1회 |
...
| RAG 및 재검색 전략 | Pre-retrieval Query Rewriting + `role=target` 사전 필터 + Dense Top-5, 웹 검색 병행(`domain` 조건 포함), 재검색 최대 1회 |
