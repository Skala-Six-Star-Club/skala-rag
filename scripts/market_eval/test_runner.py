...
주의: src/agents/market_eval/agent.py의 TODO(구조화 추출)를 담당자가 채우기
...
def build_report(scores, output_text: str) -> str:
    ...
    verdict = (
        ...
        "src/agents/market_eval/agent.py의 TODO(구조화 추출 로직)를 아직 채우지 않았다면 "
...
def main() -> None:
    ...
    output_text = str(result["market_result"])

    judge_llm = get_judge_llm()
    scores = score_with_rubric(
        judge_llm,
        target_text=output_text,
        context="market_eval(7.5절): 시장 규모/성장성/채택 현황/생태계 지지를 두 기술에 대해 대칭적으로 조사한 결과",
    )
