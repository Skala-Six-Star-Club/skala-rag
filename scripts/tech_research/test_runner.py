...
다른 에이전트의 색인·결과에는 전혀 손대지 않고 scripts/tech_research/ 아래에만 씀.

실행: python -m scripts.tech_research.test_runner
...
from scripts.tech_research.index_config import EMBEDDING_CANDIDATES
...
def run_query_rewriting_comparison(goldens: list[GoldenQuery]):
    ...
    이 에이전트 관점(perspective=general)의 골든 질의만 씀 — 리라이팅 효과는
...
현재 설정은 v1_section_aware + {ADOPTED_EMBEDDING} + Query Rewriting 적용을 기본값으로
